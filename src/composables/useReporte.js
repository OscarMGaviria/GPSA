...
/* Portada: bandera de Antioquia (blanco arriba / verde abajo) */
...
.ppt-cover-supertitle {
  font-size:13px;font-weight:700;letter-spacing:.22em;
  text-transform:uppercase;color:#9ca3af;margin-bottom:14px;
}
.ppt-cover-title {
  font-size:56px;font-weight:900;color:#0b5640;
  line-height:1.05;letter-spacing:-.02em;white-space:pre-line;
}
.ppt-cover-divider {
  width:64px;height:4px;background:#0b5640;border-radius:2px;
...
.ppt-cover-kpi {
  ...
  border-radius:12px;padding:14px 22px;text-align:center;min-width:88px;
}
.ppt-cover-kpi-val { font-size:30px;font-weight:900;color:#fff;line-height:1 }
.ppt-cover-kpi-lbl {
  font-size:10px;color:rgba(255,255,255,.55);font-weight:700;
...
.ppt-header {
  display:flex;align-items:center;justify-content:space-between;
  padding:16px 44px 14px;border-bottom:2px solid #f0f0ef;flex-shrink:0;
}
.ppt-header-title {
  font-size:20px;font-weight:800;color:#0b5640;
...
.ppt-kpi {
  flex:1;background:#f0fdf4;border:1px solid #d1fae5;border-radius:10px;
  padding:13px 16px;position:relative;overflow:hidden;
}
.ppt-kpi::before {
  content:'';position:absolute;left:0;top:0;bottom:0;
  width:3px;background:#0b5640;border-radius:3px 0 0 3px;
}
.ppt-kpi-val { font-size:28px;font-weight:900;color:#0b5640;line-height:1 }
.ppt-kpi-lbl {
  font-size:9px;color:#6b7280;margin-top:4px;font-weight:700;
...
.ppt-section-title {
  ...
  color:#9ca3af;margin-bottom:10px;padding-bottom:5px;border-bottom:2px solid #f3f4f6;
}
...
.ppt-big-kpi {
  flex:1;background:#f9fafb;border-radius:10px;padding:13px 15px;
...
.ppt-via-name {
  font-size:10px;color:#374151;flex:1;min-width:0;
...
.ppt-info-item {
  background:#f9fafb;border-radius:8px;padding:9px 13px;border:1px solid #f3f4f6;
}
.ppt-info-lbl {
...
.ppt-placeholder {
  ...
  display:flex;align-items:center;justify-content:center;
  color:#9ca3af;font-size:11px;height:200px;
}
...
// ══════════════════════════════════════════════════════════════════════════════
// PRESENTACIÓN — builders de diapositivas 16:9
// ══════════════════════════════════════════════════════════════════════════════

function pptHeaderHtml(logoUrl, title) {
  return `
  <div class="ppt-header">
    <div class="ppt-header-title">${esc(title)}</div>
    <div class="ppt-header-logo">
      <img src="${logoUrl}" class="ppt-header-logo-img" alt=""/>
      <div class="ppt-header-logo-text">
        <strong>GOBERNACIÓN DE ANTIOQUIA</strong><br>República de Colombia
      </div>
    </div>
  </div>`
}
...
function buildPptCoverSlide(logoUrl, fecha, filters, stats) {
  ...
  let mainTitle = 'PROGRAMA DE ESTABILIZACIÓN\nDE VÍAS'
  if (hasCir)  mainTitle = filters.circuito.toUpperCase()
  if (hasSub)  mainTitle = filters.subregion.toUpperCase()
  if (hasMpio) mainTitle = filters.municipio.toUpperCase()

  return `
  <div class="ppt-slide ppt-cover">
    <!-- Franja blanca superior -->
    <div class="ppt-cover-top">
      <div class="ppt-cover-top-inner">
        <div class="ppt-cover-title">${esc(mainTitle)}</div>
      </div>
    </div>
    <!-- Franja verde inferior -->
    <div class="ppt-cover-bottom">
      <div class="ppt-cover-logo-block">
        <img src="${logoUrl}" class="ppt-cover-logo-img" alt=""/>
        <div class="ppt-cover-org">GOBERNACIÓN DE ANTIOQUIA</div>
        <div class="ppt-cover-org-sub">República de Colombia</div>
      </div>
    </div>
  </div>`
}

function buildPptResumenSlide(logoUrl, stats, geoFeatures = []) {
  const subs = (stats.subregiones ?? []).filter(s => s.km > 0)
  const totalKm = subs.reduce((s, r) => s + r.km, 0) || 1
  const viasD = stats.viasDetalle ?? []
  const avgAvance = viasD.length
    ? Math.round(viasD.reduce((s, v) => s + (v.avance ?? 0), 0) / viasD.length)
    : 0

  const subsHtml = subs.map(s => {
    const p   = getSubregionPalette(s.name)
    const pct = Math.round((s.km / totalKm) * 100)
    return `
    <div class="ppt-sub-row">
      <div class="ppt-sub-name">${esc(s.name)}</div>
      <div class="ppt-sub-bar-wrap">
        <div class="ppt-sub-bar" style="width:${pct}%;background:${p.accent}"></div>
      </div>
      <div class="ppt-sub-km">${s.km.toFixed(0)} km</div>
    </div>`
  }).join('')

  const svgMap = buildSVGMap(geoFeatures, null, { accent: '#3fad72' }, 440, 310)

  return `
  <div class="ppt-slide ppt-content-slide">
    ${pptHeaderHtml(logoUrl, 'Resumen Ejecutivo — Programa de Estabilización de Vías')}
    <div class="ppt-body">
      <div class="ppt-kpi-row">
        <div class="ppt-kpi"><div class="ppt-kpi-val">${stats.viasIntervenidas}</div><div class="ppt-kpi-lbl">Tramos</div></div>
        <div class="ppt-kpi"><div class="ppt-kpi-val">${Number(stats.longitudTotal ?? 0).toFixed(0)}</div><div class="ppt-kpi-lbl">Kilómetros</div></div>
        <div class="ppt-kpi"><div class="ppt-kpi-val">${stats.municipios}</div><div class="ppt-kpi-lbl">Municipios</div></div>
        <div class="ppt-kpi"><div class="ppt-kpi-val">${stats.circuitos}</div><div class="ppt-kpi-lbl">Circuitos</div></div>
        <div class="ppt-kpi" style="background:#fffbeb;border-color:#fde68a">
          <div class="ppt-kpi-val" style="color:#d97706">${avgAvance}%</div>
          <div class="ppt-kpi-lbl">Avance promedio</div>
        </div>
      </div>
      <div class="ppt-two-col">
        <div>
          <div class="ppt-section-title">Km por subregión</div>
          <div class="ppt-sub-bars">${subsHtml}</div>
        </div>
        <div>
          <div class="ppt-section-title">Mapa de vías</div>
          ${svgMap
            ? `<div style="background:linear-gradient(155deg,#021a0e 0%,#0b5640 100%);border-radius:8px;padding:8px;display:inline-block">${svgMap}</div>`
            : '<div class="ppt-placeholder">Sin datos geográficos</div>'
          }
        </div>
      </div>
    </div>
    <div class="ppt-footer-bar"></div>
  </div>`
}

function buildPptSubregionSlide(logoUrl, subregion, vias, geoFeatures = []) {
  ...
  const circuitos = [...new Set(vias.map(v => v.circuito).filter(Boolean))]

  const svgMap = buildSVGMap(geoFeatures, subregion, p, 380, 290)

  const circRows = circuitos.slice(0, 10).map(c => {
    const cv  = vias.filter(v => v.circuito === c)
    const avg = cv.length ? Math.round(cv.reduce((s, v) => s + (v.avance ?? 0), 0) / cv.length) : 0
    const fill = avg >= 80 ? '#34d399' : avg >= 40 ? p.accent : '#f87171'
    return `
    <div class="ppt-via-row">
      <div class="ppt-via-name">${esc(c)}</div>
      <div class="ppt-via-bar-wrap"><div class="ppt-via-bar-fill" style="width:${avg}%;background:${fill}"></div></div>
      <div class="ppt-via-pct">${avg}%</div>
    </div>`
  }).join('')

  return `
  <div class="ppt-slide ppt-content-slide">
    ${pptHeaderHtml(logoUrl, `SUBREGIÓN ${esc(subregion.toUpperCase())}`)}
    <div class="ppt-body">
      <div class="ppt-circuit-kpis">
        <div class="ppt-big-kpi" style="border-top:4px solid ${p.accent}">
          ...
          <div class="ppt-big-kpi-lbl">Avance Físico Promedio</div>
        ...
        <div class="ppt-big-kpi" style="border-top:4px solid #d97706">
          ...
          <div class="ppt-big-kpi-lbl">Avance Financiero Promedio</div>
          <div class="ppt-big-progress"><div style="width:${avgAvFin}%;background:#d97706"></div></div>
        </div>
        <div class="ppt-big-kpi" style="border-top:4px solid ${p.accent}">
          <div class="ppt-big-kpi-val">${km.toFixed(0)}</div>
        ...
        <div class="ppt-big-kpi" style="border-top:4px solid #6b7280">
          <div class="ppt-big-kpi-val">${circuitos.length}</div>
          <div class="ppt-big-kpi-lbl">Circuitos</div>
        </div>
      </div>
      <div class="ppt-two-col" style="margin-top:14px">
        <div>
          <div class="ppt-section-title">Avance por circuito</div>
          <div class="ppt-via-list">${circRows || '<div class="ppt-placeholder">Sin circuitos</div>'}</div>
        </div>
        <div>
          <div class="ppt-section-title">Mapa de vías</div>
          ${svgMap
            ? `<div style="background:${p.gradient};border-radius:8px;padding:8px;display:inline-block">${svgMap}</div>`
            : '<div class="ppt-placeholder">Sin datos geográficos</div>'
          }
        </div>
      </div>
...
function buildPptCircuitoSlide(logoUrl, subregion, circuito, vias) {
  ...
  return `
  <div class="ppt-slide ppt-content-slide">
    ${pptHeaderHtml(logoUrl, `INTERVENCIÓN VIAL — ${esc(circuito)}`)}
    <div class="ppt-body">
      <div class="ppt-circuit-kpis">
      ...
      <div class="ppt-two-col" style="margin-top:14px">
        ...
        <div>
          ...
          <div class="ppt-info-grid">
            <div class="ppt-info-item">
              <div class="ppt-info-lbl">Subregión</div>
              <div class="ppt-info-val">${esc(subregion)}</div>
            </div>
            <div class="ppt-info-item">
              <div class="ppt-info-lbl">Municipio(s)</div>
              <div class="ppt-info-val">${esc(municipios.slice(0, 3).join(', ') || '—')}</div>
            </div>
            <div class="ppt-info-item">
              <div class="ppt-info-lbl">Contratista</div>
              <div class="ppt-info-val">${esc(contratistas.slice(0, 2).join(', ') || '—')}</div>
            </div>
            <div class="ppt-info-item">
              <div class="ppt-info-lbl">Plazo</div>
              <div class="ppt-info-val">${esc(plazo)}</div>
            </div>
          </div>
...
export function buildPresentacionSlides(stats, filters, logoUrl, fecha, geoFeatures = []) {
  const slides  = []
  const hasCir  = filters?.circuito  && filters.circuito  !== 'Todos los circuitos'
  const hasSub  = filters?.subregion && filters.subregion !== 'Todas las subregiones'
  const hasMpio = filters?.municipio && filters.municipio !== 'Todos los municipios'
  ...
  if (hasCir) {
    ...
    slides.push({ html: buildPptResumenSlide(logoUrl, stats, geoFeatures) })

    const subregionMap = {}
    for (const v of (stats.viasDetalle ?? [])) {
      const sub = v.subregion || 'Sin subregión'
      if (!subregionMap[sub]) subregionMap[sub] = []
      subregionMap[sub].push(v)
    }

    const subsList = hasSub
      ? [filters.subregion].filter(s => subregionMap[s])
      : hasMpio
        ? [...new Set((stats.viasDetalle ?? []).filter(v => norm(v.municipio) === norm(filters.municipio)).map(v => v.subregion))].filter(Boolean)
        : Object.keys(subregionMap).sort()
    ...
    for (const sub of subsList) {
      ...
      if (hasMpio) {
        const filtered = vias.filter(v => norm(v.municipio) === norm(filters.municipio))
        const circMap  = {}
        for (const v of filtered) {
          const c = v.circuito || v.nombre
          if (!circMap[c]) circMap[c] = []
          circMap[c].push(v)
        }
        for (const [c, cv] of Object.entries(circMap)) {
          slides.push({ html: buildPptCircuitoSlide(logoUrl, sub, c, cv) })
        }
      } else {
        slides.push({ html: buildPptSubregionSlide(logoUrl, sub, vias, geoFeatures) })
      }
    }
  }
